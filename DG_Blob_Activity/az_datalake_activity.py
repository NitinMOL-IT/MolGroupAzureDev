...
class DataLakeUtility:
    ...
    def copy_and_delete(self):
        source_container_name = "molpearlqrt"
        destination_container_name_archive = "nonprdfocusbisto/Archive Folder"
        # Get today's date
        ...
        source_blob_names = ["Online_ABLOG/DG-Boiler/ActiveVoyages.csv","Online_ABLOG/DG-Boiler/Event&OliConsumption.csv","Online_ABLOG/DG-Boiler/VesselData.csv"]
        control_file = ["Online_ABLOG_Control_File/control_file.txt"]
        ...
        try:
            ...
            for control_name in control_file:
                control_blob_client = source_container_client.get_blob_client(control_name)
                logging.info("Deleting control blob")
                control_blob_client.delete_blob()
    ...
    def missing_report_archieve(self):
        source_container_name = "nonprdfocusbisto/ABLOG_FINAL"
        destination_container_name_archive = "nonprdfocusbisto/Archive Folder"
        # Get today's date
        today = datetime.date.today()
        ...
        logging.info("Initializing Destination folder and blob names")
        destination_folder = yesterday.strftime("%Y-%m-%d")
        source_blob_names = "DG-Boiler/DGBoiler_Missing_Reports.csv"
        ...
        try:
            ...
            source_blob_client = source_container_client.get_blob_client(source_blob_names)
            destination_blob_name = f"{destination_folder}/{source_blob_names}"
            destination_blob_client = destination_container_client.get_blob_client(destination_blob_name)
        ...
        return "Missing Vessel File Archieved successfully."
